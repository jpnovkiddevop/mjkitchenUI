...
export default function Cart({cart, removeFromCart, clearCart}) {
    ...
    const myCart = cart.map((item) => (
       <li key={item.id}>
         ...
         <button className="button" onClick={() => removeFromCart(item)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
